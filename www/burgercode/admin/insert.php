...
if(!empty($_POST))
{
   ...
   if(($isSucess && $isImageUpdated && $isUploadSucccess)||($isSucess && !$isImageUpdated)
   {
    $db = Database::connect();
    if($isImageUpdated)
    {
        $statement = $db->prepare("UPDATE items name=?, price=?, category=?, image=? WHERE id =?");
  ...
  <body>
    ...
    <div class="container admin ">
      ...
      <h1>Modifier un item</h1>
        <br>
        <form class="form" role="form" action=<?php echo'update.phph?id='.$id;?> method="post" enctype="mutipart/form-data">
